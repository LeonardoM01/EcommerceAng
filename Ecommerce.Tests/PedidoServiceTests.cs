...
public class PedidoServiceTests
{
    ...
    public void CriarPedido_DeveAtribuirStatusCriado_ERegistrarHistorico()
    {
        ...
        var historicos = historicoService.ObterPorEntidade(nameof(Pedido), novoPedido.Id).ToList();
    ...
    public void PagarPedido_DeveMudarStatusParaPago()
    {
        ...
        var historicoService = new HistoricoService();
        var pedidoService = new PedidoService(historicoService);
        var pedido = pedidoService.Criar(new Pedido { ClienteId = 1 });

        // Act
        var sucesso = pedidoService.Pagar(pedido.Id);

        // Assert
        Assert.True(sucesso);
        Assert.Equal(StatusPedido.Pago, pedidoService.ObterPorId(pedido.Id)!.Status);
        
        var historicos = historicoService.ObterPorEntidade(nameof(Pedido), pedido.Id).ToList();
    ...
    public void CancelarPedido_NaoPodeCancelarSeEstiverPago()
    {
        ...
        var pedidoService = new PedidoService(historicoService);
        var pedido = pedidoService.Criar(new Pedido { ClienteId = 1 });
        pedidoService.Pagar(pedido.Id);

        // Act
        var sucesso = pedidoService.Cancelar(pedido.Id);
    ...
    public void CancelarPedido_DeveMudarStatusParaCanceladoSeNaoEstiverPago()
    {
        ...
        // Assert
        Assert.True(sucesso);
        Assert.Equal(StatusPedido.Cancelado, pedidoService.ObterPorId(pedido.Id)!.Status);
    }
}
